...
if selected == 'Check In':
    ...
    if search_term:
        if os.path.exists('Database.xlsx'):
            ...
            # Convert NaN values to empty strings before applying search
            df['ISBN'] = df['ISBN'].astype(str).fillna('')
            df['Patron'] = df['Patron'].astype(str).fillna('')

            search_results = df[df.apply(
                lambda row: search_term.lower() in row['ISBN'].lower() or search_term.lower() in row['Patron'].lower(), axis=1
            )]
    ...
    with st.form(key='check_in_form'):
        ...
        # ✅ Fixed: Submit button properly placed inside the form
        submit_button = st.form_submit_button(label='Return Book')

    if submit_button:
        if not isbn or not student_name or not year_level or not section:
            st.warning('Please fill out all required fields.')
        else:
            if os.path.exists('Database.xlsx'):
                df = pd.read_excel('Database.xlsx', dtype={'ISBN': str})
                ...
                if not matching_books.empty:
                    ...
                    if patron_list:
                        ...
                        if student_name in patrons:
                            ...
                            # Remove only the matched patron
                            patrons.pop(idx)
                            checkouts.pop(idx)
                            due_dates.pop(idx)
                            if idx < len(years):
                            ...
                            # ✅ Ensure `update_book_status(df)` is defined before calling
                            df = update_book_status(df)

                            df.to_excel('Database.xlsx', index=False)
                            st.success('Book has been checked in successfully.')
                            log_transaction('Check In', isbn, student_name, year_level, section)
                        else:
