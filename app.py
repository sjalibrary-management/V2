...
if check_password():
    ...
    if search_term:
        if os.path.exists('Database.xlsx'):
            ...
            search_results = df[df['ISBN'].astype(str).str.contains(search_term, case=False, na=False)]
            
            if not search_results.empty:
                st.dataframe(search_results[['Book Title', 'Author', 'ISBN', 'Type', 'Category', 'Publishing Date', 'Publisher', 'Patron', 'Due']], use_container_width=True)
            else:
                st.warning('No matching records found.')
        else:
            st.warning('No inventory data found.')
    ...
    with st.form(key='check_in_form'):
        ...
        date = st.date_input('Date', value=dt.today()).strftime('%Y-%m-%d')
            ...
            if not isbn or not yearLevel or not section:
                ...
                if os.path.exists('Database.xlsx'):
                    ...
                    if not matching_books.empty:
                        ...
                        # Clear the Patron field
                        df.at[book_idx, 'Patron'] = ''

                        # Also clear Due Date if necessary
                        if 'Due' in df.columns:
                            df.at[book_idx, 'Due'] = ''

                        # Update book status and save changes
                        df = update_book_status(df)
                        df.to_excel('Database.xlsx', index=False)

                        st.success('Book has been checked in successfully. Patron name has been cleared.')
                        log_transaction('Check In', isbn, yearLevel, section)
                    else:
                        st.error('Book not found in inventory.')
                else:
                    st.error('Inventory database not found.')
